...
// TestRenameRepo_RelinksLiveSymlinks is the regression test for the bug
// report: every symlink dots creates targets an absolute path built from
// the repository's own name, so renaming a repository with enabled
// namespaces must repoint every one of their destinations' symlinks as part
// of the rename — otherwise every one of them dangles until the next
// self-heal pass, racing whatever the destination's own program does when
// it finds nothing there.
func TestRenameRepo_RelinksLiveSymlinks(t *testing.T) {
	t.Setenv("XDG_STATE_HOME", t.TempDir())
	t.Setenv("XDG_CONFIG_HOME", t.TempDir())
	t.Setenv("XDG_DATA_HOME", t.TempDir())
	home := t.TempDir()
	...
	if _, enableErr := engine.Enable(key, repoDir, nsDir, "editors", entries, s, nil); enableErr != nil {
	...
	newNsDir := filepath.Join(dataDir, "renamed", "editors")
	target, err := os.Readlink(dest)
	if err != nil {
		t.Fatalf("readlink %s after rename: %v", dest, err)
	}
	if target != filepath.Join(newNsDir, "cfg") {
		t.Fatalf("dest still targets %q after rename, want it repointed into %s", target, newNsDir)
	}
	...
	entry, ok := persisted.Entries[state.Key{Repo: "renamed", Namespace: "editors"}]
	if !ok || len(entry.LinkedDests) != 1 || entry.LinkedDests[0] != dest {
		t.Fatalf("LinkedDests = %+v ok=%v, want [%s]", entry, ok, dest)
	}
}
